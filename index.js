 ...
 * @param {string} destination - The path where the file should be saved.
 * @returns {Promise} A promise that resolves when the file has been successfully decoded and saved.
 */
async function decodeBase64(base64String, destination) {
	console.log(JSON.stringify(parsedJson)); // Log parsedJson as a string
	const decodedValue = atob(parsedJson.base64String);
	// console.log(`decoded file: ${destination} \n ${decodedValue}`);
	// const response = Buffer.from(decodedValue, 'base64');
	// console.log(`decoded file: ${destination} \n ${response}`);
	await writeToFile(response, destination);
}

/**
 * Writes the response stream to the specified destination.
 *
 * @param {Stream} response - The response stream from the API.
 * @param {string} destination - The path where the file should be saved.
 * @return {Promise} A promise that resolves when the file has been successfully written.
 */
function writeToFile(response, destination) {
	return new Promise((resolve, reject) => {
